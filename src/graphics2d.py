...
def resize_with_aspect_ratio(image, target_width=None, target_height=None):
    ...
    if target_width is None and target_height is None:
        raise ValueError(
            "At least one of target_width or target_height must be provided.")
...
class ViewGrid():
    ...
    def __init__(self, size, channels=3, dtype=np.uint8):
        ...
        self.canvas = np.zeros(
            (self.width, self.height, self.channels), dtype=self.dtype)
    ...
    def get(self, idx):
        if idx < -len(self.canvas_elements) or idx >= len(self.canvas_elements):
            raise IndexError(
                f"Canvas element out of bound for index {idx} with canvas of length {len(self.canvas_elements)}")
    ...
    def grid(self, coords_abs=(0, 0, 0, 0), coords_rel=(0, 0, 0, 0), meta=None):
        ...
        if x < 0 or y < 0 or x+w >= self.width or y+h >= self.height:
            raise ValueError(
                f"Expected (x,y), (w,h) to be in range 0<x<x+w<width, 0<y<y+h<height got ({x},{y}), ({w}, {h}), for canvas of size ({self.width}), ({self.height})")

        element = ViewGridElement(
            len(self.canvas_elements), (x, y), (w, h), self, meta=meta)
    ...
    def set(self, grid_idx, new_canvas, cv2=False):
        ...
        new_canvas = ResizeModes.scale_modes[element.meta["resize"]](
            element, new_canvas)
    ...
    def fill(self, grid_idx, color=(255, 255, 255)):
        element = self.canvas_elements[grid_idx]
        new_canvas = np.ones((element.width, element.height, len(
            color)))*np.array(list(color)[::-1]).astype(self.dtype)
        self.canvas[element.x:element.x+element.width,
                    element.y:element.y+element.height, :] = new_canvas
    ...
    def draw_text(self, grid_idx, text, font_color=(255, 255, 255), font_scale=0.75):
        ...
        cv2.putText(image, text, (text_x, text_y), font,
                    font_scale, font_color, thickness=2)
...
class MainDisplay():
    def __init__(self):
        ...
        self.hirise_img = self.view.grid(coords_rel=(
            0.8, 0, 0.2, 0.5), coords_abs=(0, 80, 0, -80))
        self.baseline_img = self.view.grid(coords_rel=(
            0.8, 0.5, 0.2, 0.5), coords_abs=(0, 80, 0, -80))

        self.hirise_caption = self.view.grid(
            coords_rel=(0.8, 0, 0.2, 0), coords_abs=(0, 0, 0, 80))
        self.baseline_caption = self.view.grid(
            coords_rel=(0.8, 0.5, 0.2, 0), coords_abs=(0, 0, 0, 80))
